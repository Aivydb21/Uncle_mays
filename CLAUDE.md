...
- **Subscriptions are paused.** [src/app/subscribe/[product]/page.tsx](src/app/subscribe/[product]/page.tsx) renders a "Subscriptions are paused" banner for any slug; the underlying subscription delivery / payment routes are not in use. Existing Stripe subscriptions (e.g. Doina Romanciuc's grandfathered $55/wk) continue to bill independently, but as of 2026-05-11 the Stripe API shows zero active subscriptions — even Doina's is canceled. Do not raise "the subscription flow is broken" as a bug; it is intentionally idle.
