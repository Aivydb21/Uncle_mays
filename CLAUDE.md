...
This section exists because Paperclip CRO-style agents that can't read the repo directly have repeatedly raised "bugs" that don't exist. Before flagging any of the below as broken, verify against the live file. **Last refreshed 2026-05-11 after the architecture rewrite — earlier bullets that referenced `/checkout/[product]/*` routes, `src/lib/products.ts`, and `PRODUCTS[slug]` are gone, those structures no longer exist.**

- **One-time checkout is now a single cart-based page at `/checkout`, no product slug.** [src/app/checkout/page.tsx](src/app/checkout/page.tsx) renders [src/components/checkout/CheckoutClient.tsx](src/components/checkout/CheckoutClient.tsx) which reads the cart from [src/lib/cart/store.ts](src/lib/cart/store.ts), pulls active pickup/delivery slots via [src/lib/catalog/pickup-slots.ts](src/lib/catalog/pickup-slots.ts), and uses embedded Stripe Elements (`PaymentElement` via `@stripe/react-stripe-js`) — not Stripe-hosted Checkout redirect. **Routes like `/checkout/small`, `/checkout/family`, `/checkout/[product]/delivery`, `/checkout/[product]/payment` no longer exist and will 404.**
- **Subscriptions are paused — `/subscribe/*` 308s to `/shop` server-side (UNC-1210, 2026-05-19).** [next.config.ts](next.config.ts) `redirects()` returns `{ source: "/subscribe/:path*", destination: "/shop", permanent: true }`, so `/subscribe`, `/subscribe/starter`, `/subscribe/family`, `/subscribe/community`, and any future slug return HTTP 308 to `/shop` with all query params preserved (utm_*, gclid, fbclid, fbp, fbc, ga_client_id, promo). The legacy [src/app/subscribe/[product]/page.tsx](src/app/subscribe/[product]/page.tsx) "Subscriptions are paused" banner still exists in the repo but is unreachable. Existing Stripe subscriptions (e.g. Doina Romanciuc's grandfathered $55/wk) bill independently of any URL; as of 2026-05-11 the Stripe API shows zero active subscriptions. Do not raise "the subscription flow is broken" as a bug; it is intentionally idle.
- **Catalog is Airtable-driven, not a static `products.ts`.** Product data flows from Airtable (base `app3raEVB9kHeUoHE` Product Mix) via [src/lib/catalog/airtable.ts](src/lib/catalog/airtable.ts) and is shaped by [src/lib/catalog/types.ts](src/lib/catalog/types.ts). `src/lib/products.ts` and the `PRODUCTS[slug]` constant **do not exist**. Pricing math lives in [src/lib/cart-pricing.ts](src/lib/cart-pricing.ts) and [src/lib/cart-pricing-constants.ts](src/lib/cart-pricing-constants.ts).
- **Delivery cadence is daily, citywide, customer-picked window, no cutoff (2026-05-10).** The earlier "Order by Sunday 11:59 PM CT for Wednesday delivery" amber banner is retired. If you see Sunday/Wednesday copy anywhere on a checkout-flow file, flag it as stale. Customer-visible source of truth is `customer-facts.md`.
...
- **`PATCH /api/checkout/session` is self-healing on missing session (UNC-1204, 2026-05-19).** [src/app/api/checkout/session/route.ts](src/app/api/checkout/session/route.ts) returns `200 { session: null, lost: true, sessionId }` (not 404) when `updateSession()` can't find the record. The underlying [src/lib/checkout-store.ts](src/lib/checkout-store.ts) writes to `process.cwd()/data/checkout-sessions.json`, which is read-only / ephemeral on Vercel — writes silently drop. PATCH callers (`CheckoutClient.tsx` post-Stripe markers, `sms-confirmation.ts` marker) ignore the response body. Do not raise "PATCH returns 404" as a bug; the durable-store fix (KV / Upstash / Stripe metadata) is a separate board-approved initiative.
