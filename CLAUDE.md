...
- **One-time checkout is a 2-step flow (UNC-648).** `/checkout/[product]` (Step 1) now shows product summary + delivery form on a single page; `/checkout/[product]/payment` (Step 2) handles Stripe payment. The old 3-step flow with a separate `/checkout/[product]/delivery` route still exists as a file but is no longer linked in the main navigation. Email is captured in the delivery section of Step 1, not behind a separate gate.
...
- **Delivery forms validate Chicago-only delivery.** [src/app/subscribe/[product]/delivery/page.tsx](src/app/subscribe/[product]/delivery/page.tsx), the merged one-time checkout page [src/app/checkout/[product]/page.tsx](src/app/checkout/[product]/page.tsx), and the legacy [src/app/checkout/[product]/delivery/page.tsx](src/app/checkout/[product]/delivery/page.tsx) all require state=IL and ZIP starting with `606`. Out-of-area visitors see an inline email capture waitlist form (via `WaitlistCapture` component) that posts to `/api/capture-email`.
- **Address autocomplete (Google Places).** The merged one-time checkout page, the subscription delivery page, and the legacy checkout delivery page all use `useAddressAutocomplete` hook ([src/hooks/use-address-autocomplete.ts](src/hooks/use-address-autocomplete.ts)) to attach Google Places Autocomplete to the street address field. On selection, city/state/ZIP auto-populate. Requires `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` in env; gracefully degrades to manual entry when key is missing.
