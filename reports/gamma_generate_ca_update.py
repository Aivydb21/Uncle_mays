"""Generate the Cleveland Avenue Founder Update No. 1 in Gamma via the public REST API."""
import json
import os
import sys
import time
...
DECK = os.path.join(
    ...
    "investor-outreach", "materials", "cleveland-avenue-update-2026-05",
    "cleveland-avenue-update.md",
)
...
THEME_PROMPT = (
    "Investment banker founder update style. Conservative palette: deep navy "
    ...
    "Confidential. Prepared for Cleveland Avenue. May 2026.' in 9pt muted gray, "
    "right-aligned with page number. Reference register: institutional founder "
    "update letter, no sales tone."
)
...
while time.time() < deadline:
    ...
    if state in ("completed", "succeeded", "done"):
        ...
        url = body.get("gammaUrl") or body.get("url")
        if url:
