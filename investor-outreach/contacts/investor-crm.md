# Investor Pipeline Tracker

> Last updated: 2026-04-06

## Pipeline Summary

| Stage | Count | Capital |
|-------|-------|---------|
| Prospect | 87 | - |
| Contacted | 10 | - |
| Meeting Scheduled | 0 | - |
| Due Diligence | 0 | - |
| Committed | 0 | - |
| **Total** | **97** | **$0 / $500K** |

### Prioritized Pipeline (generated 2026-04-05, full database scored)

| Tier | Count | Description |
|------|-------|-------------|
| Tier 1 | 66 (69 in Apollo campaign) | Thesis-aligned equity investors, target metros (SF, LA, NYC, DC, Chicago), senior decision-makers |
| Tier 2 | 612 | Strong-fit equity investors (national) + Tier 1 overflow |
| Tier 3 | 677 | General investors (national), hold for future campaign |
| LinkedIn Tier 1 | 99 | Thesis-aligned, national (no geo filter), for LinkedIn outreach |
| **Total scored** | **1,355** | Out of 3,108 unique contacts in full Apollo database (1,753 excluded) |

> Full lists: `pipeline/tier-1/priority-contacts.md`, `tier-2/warm-contacts.md`, `tier-3/campaign-contacts.md`

---

## Active Pipeline

| Investor | Organization | Type | Status | Amount | Last Touch | LI Status | Next Action | Notes |
|----------|-------------|------|--------|--------|------------|-----------|-------------|-------|
| Marianne Wu | Congruent Ventures | VC | Pass | - | 2026-04-08 | -- | None — pass | Board Member & Investor. Apollo Step 1 sent 4/6. Replied 4/6: climate thesis, not a fit. Grace reply drafted 4/8 (draft ID: r6170193910360179612). |
| Tyler Mayoras | Manna Tree | VC | Contacted | - | 2026-04-08 | Connected | DM1 ~Apr 15 | MD, Food & Beverage focus. Gmail sent 4/4, LI requested 4/6, connected 4/8. |
| James Norman | Black Operator Ventures | VC | Contacted | - | 2026-04-06 | Requested | Await reply | MP, Black-led operator-focused fund. Gmail sent 4/4, LI requested 4/6. |
| Jordan Gaspar | AF Ventures | VC | Contacted | - | 2026-04-06 | Requested | Await reply | GP, food/ag VC. Gmail sent 4/4, LI requested 4/6. |
| Sonia Nagar | SNAK Venture Partners | VC | Contacted | - | 2026-04-06 | Requested | Await reply | Founder & MP, food-focused VC. Gmail sent 4/4, LI requested 4/6. |
| Corey Vernon | Radicle Impact Partners | Impact | Contacted | - | 2026-04-06 | Requested | Await reply | Partner, impact investor. Gmail sent 4/4, LI requested 4/6. |
| Rodney Clark | Aspect Consumer Partners | VC | Contacted | - | 2026-04-06 | Requested | Await reply | MP & Co-Founder, consumer-focused. Gmail sent 4/4, LI requested 4/6. |
| Devon Roshankish | Forward Consumer Partners | VC | Pass | - | 2026-04-08 | -- | None — pass | Investor, consumer-focused. Gmail sent 4/4, LI requested 4/6. Replied 4/8: not a fit. |
| Michael Rauenhorst | SIMA Funds | Impact | Contacted | - | 2026-04-06 | Requested | Await reply | MP, social impact fund. Gmail sent 4/4, LI requested 4/6. |
| Tim Kramer | Chicago Capital Partners | VC | Contacted | - | 2026-04-06 | Requested | Await reply | MD, Chicago-based. Gmail sent 4/4, LI requested 4/6. |
| Hannah Schiff | DWM | Impact | Contacted | - | 2026-04-06 | Requested | Await reply | MD Impact. Gmail sent 4/4, LI requested 4/6. |

### Status Key
- **Prospect** - Identified, not yet contacted
- **Contacted** - Initial outreach sent (via Apollo or direct)
- **Meeting** - Meeting scheduled or completed
- **DD** - In due diligence / reviewing materials
- **Committed** - Verbal or written commitment
- **Closed** - SAFE signed and funded
- **Pass** - Replied with decline; no further automated outreach

### LI Status Key
- **--** - No LinkedIn action taken
- **Viewed** - Profile viewed
- **Requested** - Connection request sent
- **Connected** - Connection accepted
- **DM1 Sent** - First DM sent (Day 7)
- **DM2 Sent** - Second DM sent (Day 14)
- **InMail Sent** - InMail sent (Day 22, non-connected only)
- **LI Reply** - Replied via LinkedIn

### Type Key
- **Angel** - Individual angel investor
- **VC** - Venture capital fund
- **Impact** - Impact/social impact fund
- **CDFI** - Community development financial institution
- **Family** - Family office
- **Corp** - Corporate/strategic investor
