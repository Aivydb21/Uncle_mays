#!/usr/bin/env python3
"""
Create UPDATED Gmail drafts with awards/SXSW credentials for PR pitch emails
"""
...
# UPDATED Email 1: Ally Marotti (Crain's) with awards
email1_to = 'ally.marotti@crain.com'
email1_subject = "Chicago story: Award-winning grocer challenging food retail norms, $6.3M Year 1 revenue"
email1_body = """Hi Ally,
...
The fundamentals are unusual for grocery. I'm a Chicago Booth MBA with a PE/M&A background, and we won the 2025 Chicago TechRise Award and World Business Chicago Food Innovation Award for the model. We've secured a $2M SBA loan from Busey Bank and we're projecting $6.3M in Year 1 revenue at 35% gross margins. The store is effectively a data platform, the first built specifically to understand and serve Black food consumption patterns. I spoke about the broader food systems implications at South by Southwest this year.

We have architectural plans from Civic Projects and Food Market Designs, and we're finalizing our Hyde Park location. The broader vision is an 82-store rollout across historically Black American communities nationwide, but the Chicago story is the starting point.
...
# UPDATED Email 2: Derek Dingle (Black Enterprise) with awards
email2_to = 'derek.dingle@blackenterprise.com'
email2_subject = "Award-winning Black entrepreneur: First data platform for Black food consumption, SXSW speaker"
...
# Create both UPDATED drafts
print("Creating UPDATED Gmail drafts with awards/SXSW credentials...")
draft1 = create_draft(email1_to, email1_subject, email1_body)
print(f"[OK] UPDATED draft for Ally Marotti (Crain's) - Draft ID: {draft1['id']}")

draft2 = create_draft(email2_to, email2_subject, email2_body)
print(f"[OK] UPDATED draft for Derek Dingle (Black Enterprise) - Draft ID: {draft2['id']}")

print("\n[SUCCESS] Both UPDATED drafts created!")
print("\nNOTE: These are NEW drafts with awards/SXSW credentials.")
print("Previous drafts (without awards) are still in Gmail and should be deleted.")
print("Use these UPDATED versions instead.")
